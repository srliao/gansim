...
func Calc(p Profile, s Set) []DmgResult {

	artifactStats := s.TotalStats()
	baseAtk := p.CharBaseAtk + p.WeaponBaseAtk

	var r []DmgResult

	for _, ab := range p.Abilities {
		var totalAtk, atk, atkp, cc, cd, elep, em, dmgBonus, reactionBonus, defShred, defAdj, resAdj, res float64
		totalAtk = baseAtk
		...
		vmMult := ab.VapMeltMultiplier*(1+0.00189266831*em*math.Exp(-0.00505*em)) + reactionBonus
